...
public class LayerValidatorImpl implements LayerValidator {
	...
	public DefaultFeatureCollection validateConIntersected() throws SchemaException {
		SimpleFeatureCollection sfc = validatorLayer.getSimpleFeatureCollection();
		...
		if (tmpsSimpleFeatures.size() != 0) {
			if (tmpsSimpleFeatures.size() < 25000) {
				treadNum = 1;
			} else {
				treadNum = tmpsSimpleFeatures.size() / 25000;
			}
		} else
			return null;
		...
		class Task implements Runnable {
			...
			public void run() {
				...
				for (int i = startNum; i < endNum - 1; i++) {
					...
					System.out.println(tmpSimpleFeatureI.getID());
		...
		if (conIntersectedResult.errDf.size() > 0) {
			return conIntersectedResult.errDf;
		} else {
			return null;
		}
	}
	...
	class ConIntersectedResult {
		...
		DefaultFeatureCollection errDf = new DefaultFeatureCollection();

		synchronized void addErrorFeature(ErrorFeature errorFeature) {

			Geometry geom = errorFeature.getErrPoint();
			String featureId = errorFeature.getFeatureID();
			try {
				SimpleFeatureType simpleFeatureType = DataUtilities.createType("errFeature." + errCount,
						"the_geom:" + geom.getGeometryType() + ",featureID:String,errName:String:");
				Object[] objects = new Object[3];
				objects[0] = geom;
				objects[1] = featureId;
				objects[2] = errorFeature.getErrName();
				SimpleFeature simpleFeature = SimpleFeatureBuilder.build(simpleFeatureType, objects,
						"errFeature." + errCount);
				errDf.add(simpleFeature);
				errCount++;
			} catch (SchemaException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
